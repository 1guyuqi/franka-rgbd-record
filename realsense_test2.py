...
def depth_aligned_to_cloud(depth_u16, color_bgr, depth_scale, intr, zmin=0.05, zmax=2.0):
    ...
    rgb = color_bgr.reshape(-1, 3)[m][:, ::-1].astype(np.float64) / 255.0  # BGR->RGB
    return pts, rgb
...
def main():
    ...
    FPS = int(rs_cfg["fps"])
    WARMUP = int(rs_cfg.get("warmup_frames", 10))

    OUT_DIR = cfg["output"]["out_dir"]
    os.makedirs(OUT_DIR, exist_ok=True)

    T_CAM2BASE = np.asarray(cfg.get("cam2base", np.eye(4)), dtype=np.float64)
    if T_CAM2BASE.shape != (4, 4):
        raise ValueError(f"cam2base must be 4x4, got {T_CAM2BASE.shape}")

    cloud_cfg = cfg.get("cloud", {})
    ZMIN = float(cloud_cfg.get("zmin", 0.05))
    ZMAX = float(cloud_cfg.get("zmax", 2.0))
    ...
    colorizer = rs.colorizer()  # create once
    ...
    try:
        while True:
            ...
            depth_vis = np.asanyarray(colorizer.colorize(df).get_data())
            ...
            if k == ord("a"):
                ...
                pts_cam, cols = depth_aligned_to_cloud(depth_u16, color_bgr, depth_scale, intr, zmin=ZMIN, zmax=ZMAX)
