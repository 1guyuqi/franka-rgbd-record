...
POS_TARGET = np.array([0.7, 0.2, 0.2], dtype=np.float64)
...
def main():
    ...
    try:
        ...
        hold_n = int(max(1, round(HOLD_TIME_S / dt)))
        for _ in range(hold_n):
            ctrl.set_control(pos_cmd, quat_cmd)
            time.sleep(dt)

        # -------------------------
        # 3) OUTER-LOOP ITERATIVE CONVERGENCE (NEW)
        #    Update pos_cmd so that measured pos_now converges to POS_TARGET.
        # -------------------------
        ...
        # Final hold at the *bias-compensated* command
